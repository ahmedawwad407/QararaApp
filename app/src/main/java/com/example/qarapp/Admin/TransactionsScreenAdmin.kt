...
import kotlinx.android.synthetic.main.activity_complaints_screen.sent_btn_sent
import kotlinx.android.synthetic.main.activity_transactions_screen_admin.*
import java.util.*

class TransactionsScreenAdmin : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_transactions_screen_admin)
        db = Firebase.firestore
        Btn3.setOnClickListener {
            add_Complaints(
                CounterNum.text.toString(),
                ConfirmeCounterNum.text.toString(),
            )
        }
    ...
    private fun add_Complaints(
        ...
        var Complaints = hashMapOf(
            "transaction_number" to transaction_number,
            "year" to year,
            "created_at" to com.google.firebase.Timestamp(Date()),
        )
        db.collection("follow_Transactions").add(Complaints)
            .addOnSuccessListener { documentReference ->
                Toast.makeText(this, "تم اضافة متابعة المعاملات ", Toast.LENGTH_SHORT).show()
